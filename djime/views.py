...
from djime.forms import SlipAddForm, SlipChangeForm, TimeSliceForm
...
def slip_action(request, slip_id, action):
    ...
    if action == 'start':
        ...
        if not TimeSlice.objects.filter(user=request.user,
            ...
            if slice_query_set:
                for time_slice in slice_query_set:
                    time_slice.end = datetime.now()
                    time_slice.save() # updates duration and saves the timeslice using signals.py

            new_slice = TimeSlice.objects.create(user = request.user, begin = start_time, slip_id = slip_id )
            new_slice.save()
        ...
        time_slice.save()
        return HttpResponse(trans('Your timeslice for slip "%(name)s", begintime %(begin)s has been stopped at %(end)s') % {'name': time_slice.slip.name, 'begin': time_slice.begin, 'end': time_slice.end})
...
def time_slice_create(request, slip_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            new_slice = form.save(commit=False)
            new_slice.user = request.user
            new_slice.slip = slip
            new_slice.save()
    ...
    if request.method == 'GET':
        ...
        return render_to_response('djime/time_slice_create.html',
                                  {'time_slice_add_form': TimeSliceForm(),
                                   'slip': slip,
                                  },
                                  context_instance=RequestContext(request))
...
def time_slice(request, slip_id, time_slice_id):
    ...
    if request.method == 'GET':
        ...
        form = TimeSliceForm(request.POST, instance=time_slice)
        if form.is_valid():
