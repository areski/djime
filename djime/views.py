...
def slip_action(request, slip_id, action):
    ...
    if action == 'start':
        ...
        if not TimeSlice.objects.filter(user=request.user,
            ...
            new_slice = TimeSlice.objects.create(user = request.user, begin = start_time, slip_id = slip.id )
            new_slice.save()
        ...
        time_slice = TimeSlice.objects.get(user = request.user, slip = slip.id, end = None)
