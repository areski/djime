...
$.getJSON('/project/json/', function(data) {
	...
	$(document).ready(function () {
		...
		if ($(".timesheet #id_project").length) {
			get_slip_data = function() {
				$.post(document.URL, {'project': $(".timesheet #id_project").val()}, function(data) {
					$(".timesheet #id_slip").html(data).show();
				});
			}
			$(".timesheet #id_project").change(function () {
				get_slip_data()
			});
			$(".timesheet #id_project").trigger('change');
		}
	});
});
